...
def calculate_tokens(conversation, processor, tokenizer):
    ...
    for message in conversation["conversations"]:
        ...
        encode_id = tokenizer.apply_chat_template(
            conv, return_tensors="pt", add_generation_prompt=False
        )[0]
        total_tokens += len(encode_id)
    if "image" in conversation:
        images = (
            conversation["image"]
            if isinstance(conversation["image"], list)
            else [conversation["image"]]
        )
        ...
        videos = (
            conversation["video"]
            if isinstance(conversation["video"], list)
            else [conversation["video"]]
        )
...
for dataset_name, config in datasets.items():
    ...
    annotation_path = os.path.join(
        processor.data_args.data_path, config["annotation_path"]
    )
    ...
    count_file_path = annotation_path.replace(".jsonl", "_count.json").replace(
        ".json", "_count.json"
    )
    if os.path.exists(count_file_path):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            data_with_tokens = list(
                tqdm(
                    executor.map(calculate_and_update, data),
                    total=len(data),
                    desc=f"Processing {dataset_name} data",
                )
            )
    ...
    pack_output_path = annotation_path.replace(".jsonl", "_pack.json").replace(
        ".json", "_pack.json"
    )
