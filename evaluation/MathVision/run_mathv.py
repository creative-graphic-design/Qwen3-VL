...
from dataset_utils import dump_image, load_dataset
from eval_utils import MATH_V_acc, build_judge, eval_single_sample
...
def clean_dataframe_for_excel(df):
    ...
    return (
        df.applymap(clean_for_excel)
        if hasattr(df, "applymap")
        else df.map(clean_for_excel)
    )
...
def run_inference(args):
    ...
    if args.num_samples is not None and args.num_samples > 0:
        ...
        print(
            f"✓ Loaded {len(data)} samples from {args.dataset} (limited from {original_len} samples)"
        )
    else:
    ...
    if args.use_cot:
        cot_prompt = (
            args.cot_prompt if args.cot_prompt else " Let's think step by step."
        )
    ...
    if sampling_params.presence_penalty > 0:
        print(
            f"   ✅ Anti-repetition enabled (presence_penalty={sampling_params.presence_penalty})"
        )

    if sampling_params.temperature <= 0.02 and sampling_params.top_k == 1:
        print("   ✅ Using FAST greedy-like decoding")
    else:
        print("   ⚠️  Using sampling decoding (slower but more diverse)")
    print()
    ...
    for idx, (_, line) in enumerate(
        tqdm(data.iterrows(), total=len(data), desc="Building prompts")
    ):
    ...
    for idx, (line_dict, messages, output) in enumerate(
        zip(all_line_dicts, all_messages, outputs)
    ):
...
def run_evaluation(args):
    ...
    output_xlsx = (
        args.output_file.replace(".csv", ".xlsx")
...
def main():
    ...
    infer_parser.add_argument(
        "--data-dir", type=str, help="The absolute path of MathVision data directory"
    )
    infer_parser.add_argument(
