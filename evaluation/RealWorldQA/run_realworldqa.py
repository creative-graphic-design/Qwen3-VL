...
def run_inference(args):
    ...
    if args.data_dir:
        ...
        raise ValueError(
            "Please specify --data-dir or set LMUData environment variable"
        )
    ...
    for idx, (_, line) in enumerate(
        ...
        # Build prompt
        messages = build_realworldqa_prompt(
            line, dump_image_func, min_pixels, max_pixels
        )
...
def run_evaluation(args):
    ...
    if args.eval_model:
        model = build_judge(
            model=args.eval_model, api_type=getattr(args, "api_type", "dash")
        )
    ...
    print(f"{'=' * 50}")
    print(
        f"Overall accuracy: {accuracy:.4f} ({sum(r['hit'] for r in eval_results)}/{len(eval_results)})"
    )
    print(f"{'=' * 50}\n")
...
def main():
    ...
    infer_parser.add_argument(
        "--dataset", type=str, default="RealWorldQA", help="Dataset name"
    )
